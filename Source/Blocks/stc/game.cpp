...
#ifdef STC_DEBUG_STRING
// Return a debug string of the board
std::string Game::getDebugString()
{
    std::stringstream ss;
    for (int k = 0; k < BOARD_TILEMAP_WIDTH; ss << "-", k++) { }
    ss << "\n";
    for (int y = 0; y < BOARD_TILEMAP_HEIGHT; y++)
    {
        for (int x = 0; x < BOARD_TILEMAP_WIDTH; x++)
        {
            int tx = x - fallingBlock().x;
            int ty = y - fallingBlock().y;
            if ((tx >= 0) && (tx < TETROMINO_SIZE) && (ty >= 0) && (ty < TETROMINO_SIZE) 
                          && fallingBlock().cells[tx][ty] != Game::EMPTY_CELL)
            {
                ss << "x";
            }
            else
            {
                ss << ((mMap[x][y] == EMPTY_CELL) ? " " : "*");
            }
        }
        ss << "\n";
    }
