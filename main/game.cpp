...
#ifdef DEBUG  
#include <stdio.h>
#define debugPrint( ... ) do{ char s[80]; sprintf( s, __VA_ARGS__ ); mPlatform->println( s ); } while( 0 );
#else
#define debugPrint( ... )
#endif
...
bool Game::checkCollision(int dx, int dy)
{
    ...
    debugPrint( "checkCollision: %d %d", newx, newy );
...
void Game::moveTetromino(int x, int y)
{
    ...
    else
    {
        ...
        mFallingBlock.x += x;
        mFallingBlock.y += y;
        debugPrint( "mFallingBlock: %d %d", mFallingBlock.x, mFallingBlock.y );
    }
    onTetrominoMoved();
}
